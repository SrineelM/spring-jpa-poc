...
/**
 * This configuration class defines a custom thread pool for handling asynchronous tasks. Using a
 * custom executor is a best practice for production applications, as it allows you to fine-tune the
 * thread pool settings to match your application's workload and prevents resource contention with
 * other parts of the framework.
 *
 * <p>Key features: - Custom ThreadPoolTaskExecutor with configurable pool sizes and queue capacity
 * - Rejection policy with metrics tracking for monitoring saturation - Graceful shutdown to allow
 * in-flight tasks to complete - Comprehensive metrics binding for observability
 */
@Configuration // Marks this class as a source of bean definitions
@EnableAsync // Enables Spring's asynchronous method execution capability, allowing @Async
// annotations to work
public class AsyncConfig {

  /**
   * Creates a custom ThreadPoolTaskExecutor bean named "taskExecutor". This executor will be used
   * for all methods annotated with @Async. It includes metrics binding for monitoring thread pool
   * performance.
   *
  ...
  public Executor taskExecutor(MeterRegistry meterRegistry) {
    ...
    final Counter rejectionCounter =
        Counter.builder("async.executor.rejections")
    ...
    Tags commonTags =
        Tags.of("executor", "taskExecutor", "app", "spring-jpa-poc", "poolType", "threadPool");

    // Bind standard JVM executor metrics (e.g., active threads, completed tasks, etc.)
    ExecutorServiceMetrics.monitor(
        meterRegistry, executor.getThreadPoolExecutor(), "async.executor", commonTags);

    // Custom gauges for quick dashboards (ExecutorServiceMetrics already supplies many, these are
    // illustrative):
    // Gauge for current queue size - useful for monitoring backlog
    Gauge.builder(
            "async.executor.queue.size", () -> executor.getThreadPoolExecutor().getQueue().size())
    ...
    // (Alerting Guidance) Alert when active/core or queue.size/capacity exceed sustained thresholds
    // or on any rejection.

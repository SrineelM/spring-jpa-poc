...
/**
 * Configuration for structured logging and tracing. This class sets up a filter that adds
 * contextual information to each HTTP request, enabling better observability and debugging through
 * correlated logs.
 *
 * <p>Key features: - Generates unique request IDs for tracing - Adds client information to MDC
 * (Mapped Diagnostic Context) for structured logging - Sets response headers for correlation -
 * Handles IP address extraction with proxy support - Cleans up MDC to prevent memory leaks
 */
@Configuration // Marks this class as a source of bean definitions for Spring
public class LoggingConfig {

  /**
   * Creates a OncePerRequestFilter bean that adds logging context to each HTTP request. This filter
   * runs once per request and is ideal for adding request-scoped information.
   *
  ...
  public OncePerRequestFilter requestTrackingFilter() {
    return new OncePerRequestFilter() {
      /**
       * Processes each HTTP request to add logging context. This method is called for every
       * incoming request.
       *
      ...
      protected void doFilterInternal(
        ...
        String sessionId =
            session != null ? session.getId() : "N/A"; // (Prod) Omit or hash if considered PII.
        ...
        try {
          ...
          // (Security) Consider normalizing / hashing userAgent for privacy if logs exported to
          // third parties.
          MDC.put(
              "userAgent", userAgent != null ? userAgent : "unknown"); // Client browser/app info
          ...
          // (Prod) Add security headers here or in a dedicated filter (e.g. CSP, Referrer-Policy,
          // Permissions-Policy)
          // response.setHeader("Content-Security-Policy", "default-src 'self'"); // Example
          // placeholder
      ...
      /**
       * Extracts the client's real IP address, considering proxy headers. This method handles
       * common proxy scenarios like load balancers or CDNs.
       *
       * @param request The HTTP servlet request
       * @return The client's IP address as a string
       */
      private String getClientIpAddress(HttpServletRequest request) {
        // (Security) In production, validate trusted proxy chain before honoring X-Forwarded-For to
        // avoid spoofing.

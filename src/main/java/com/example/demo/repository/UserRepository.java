...
public interface UserRepository extends JpaRepository<User, Long>, JpaSpecificationExecutor<User> {
  ...
  /**
   * Finds a user by their email and eagerly fetches their profile in the same query. This is an
   * example of solving the N+1 problem for a one-to-one relationship using a LEFT JOIN FETCH
   * clause.
   *
  ...
  /**
   * Solves the N+1 problem by fetching all users and their posts in a single query using JOIN
   * FETCH. This is a good approach when you know you will always need the posts. The DISTINCT
   * keyword is important to avoid duplicate User records in the result set due to the join.
   *
  ...
  /**
   * Solves the N+1 problem using an @EntityGraph. This approach is cleaner than JOIN FETCH as it
   * separates the query from the fetch plan. The entity graph "User.withProfileAndPosts" is defined
   * in the User entity. Spring Data JPA will automatically generate the appropriate join query
   * based on the graph.
   *
  ...
  /**
   * Demonstrates a DTO projection. This query fetches only the id, name, and email from the User
   * entity and directly instantiates a UserSummaryDto for each result. This is highly efficient
   * because it avoids fetching the entire entity and its associations, reducing the amount of data
  ...
  int updateName(
      @Param("id") Long id, @Param("name") String name); // bulk update (returns rows affected)

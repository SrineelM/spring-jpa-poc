...
public class JwtTokenProvider {
  ...
  private Key key() {
    // (Security) Secret should be 64+ bytes for HS512 and sourced from environment / vault, not
    // plain properties.
    // (Key Rotation) For production, implement a KeyResolver that selects signing/verification keys
    // by key id (kid) header.
    // Example approach (omitted for brevity): maintain Map<String, Key> activeKeys; add
    // .setHeaderParam("kid", currentKeyId) when building token.
  ...
  public String generateToken(Authentication authentication) {
    ...
    Date expireDate =
        new Date(
            currentDate.getTime() + jwtExpirationDate); // compute absolute expiration timestamp
  ...
  public boolean validateToken(String token) {
    try {
      ...
      if (claims
          .getExpiration()
          .before(
              new Date())) { // explicit expiration check (library also throws ExpiredJwtException)

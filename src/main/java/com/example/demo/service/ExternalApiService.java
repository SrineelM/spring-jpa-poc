...
public class ExternalApiService {
  ...
  public CompletableFuture<String> fetchData() {
    return CompletableFuture.supplyAsync(
        () -> {
          ...
          try {
            logger.info(
                "Starting external API call #{} with correlation ID: {}", requestId, correlationId);

            // Simulate network latency
            Thread.sleep(
                ThreadLocalRandom.current().nextInt(100, 500)); // variable simulated latency

            // Simulate random failures (30% failure rate for demonstration)
            if (ThreadLocalRandom.current().nextDouble()
                < 0.3) { // inject failure for resilience demonstration
            ...
            String response =
                "External API Response #"
                    + requestId
                    + " at "
                    + System.currentTimeMillis(); // synthetic payload

            logger.info(
                "External API call #{} completed successfully. Response: {}", requestId, response);
            // (Metrics) Tag outcome=success manually via custom Counter/Timer if finer-grained
            // labels needed.
            return response;
  ...
  public CompletableFuture<String> fallback(Throwable t) {
    ...
    logger.error(
        "Fallback executed for external API call. Correlation ID: {}, Error: {}",
        correlationId,
        t.getMessage());
  ...
  @TimeLimiter(name = "myTimeLimiter")
  @Timed(
      value = "external.api.health.duration",
      description = "Time taken for external API health check")
    ...
    return CompletableFuture.supplyAsync(
        () -> {
          try {
            ...
            logger.debug(
                "External service health check result: {}", healthy ? "HEALTHY" : "UNHEALTHY");
            return healthy;
  ...
  public ServiceStats getServiceStats() {
    return new ServiceStats(
        requestCounter.get(), successCounter.get(), failureCounter.get(), calculateSuccessRate());
  }
  ...
  // (Observation) For production, expose these counters as Gauges/FunctionCounters or restructure
  // using MeterRegistry injection.
  ...
  public static class ServiceStats {
    ...
    public ServiceStats(
        int totalRequests, int successfulRequests, int failedRequests, double successRate) {

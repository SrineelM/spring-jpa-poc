 ...
 * This approach is useful when you need more fine-grained control over transactions than what
 * declarative annotations (@Transactional) can provide.
 */
@Service
public class ProgrammaticTransactionService {

  private static final Logger logger =
      LoggerFactory.getLogger(ProgrammaticTransactionService.class);
  ...
  public void createUserSuccessfully(User user) {
    ...
    transactionTemplate.execute(
        status -> {
          logger.info(
              "Executing successful programmatic transaction for user: {}", user.getEmail());
          userRepository.save(user);
          logger.info(
              "User {} saved successfully within programmatic transaction.", user.getEmail());
  ...
  public void createUserAndRollback(User user) {
    logger.info(
        "Attempting to create user {} in a transaction that will be rolled back.", user.getEmail());
    try {
      transactionTemplate.execute(
          status -> {
            logger.info(
                "Executing programmatic transaction for user: {}, expecting rollback.",
                user.getEmail());
            userRepository.save(user);
            logger.info(
                "User {} saved, but now throwing exception to trigger rollback.", user.getEmail());
            // By throwing a RuntimeException, we signal to the TransactionTemplate that the

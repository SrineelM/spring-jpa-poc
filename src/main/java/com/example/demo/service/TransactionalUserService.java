...
/**
 * This service demonstrates various declarative transaction management features in Spring using
...
public class TransactionalUserService {
    ...
    /**
     * Demonstrates the rollbackFor attribute. By default, Spring only rolls back transactions for
     * unchecked exceptions (RuntimeException). Here, we configure it to also roll back for our custom
    ...
    /**
     * Demonstrates the readOnly attribute. The readOnly flag is a hint to the persistence provider
     * that this transaction will not perform any write operations. This can enable performance
     * optimizations at the JDBC driver, database, or JPA provider level.
     */
    ...
    /**
     * This method demonstrates propagation behavior by calling a method with a different propagation
     * level.
     */
    @Transactional
    public void demonstratePropagation(User user1, User user2) {
        ...
        try {
            // We call the method on the proxied `self` instance to ensure the @Transactional annotation
            // on the called method is honored.
            self.createUserWithRequiresNew(user2);
        } catch (Exception e) {
            logger.error(
                    "Exception caught in outer transaction: {}. The outer transaction will still commit" + " user1.",
                    e.getMessage());
        }
    ...
    /**
     * This method demonstrates the REQUIRES_NEW propagation level. When this method is called, Spring
     * suspends the current transaction (if one exists) and starts a completely new, independent
     * transaction. This is useful when you need to ensure an operation commits or rolls back
    ...
    public void createUserWithRequiresNew(User user) {
        ...
        // Uncomment the line below to see how a failure in the inner transaction does not affect the
        // outer transaction.
        // throw new RuntimeException("Simulating failure in inner transaction");
    }

    /**
     * This method demonstrates the isolation level attribute. Isolation levels define how
     * transactions are isolated from each other, preventing concurrency issues like dirty reads,
     * non-repeatable reads, and phantom reads. SERIALIZABLE is the highest isolation level, providing
     * the most protection but potentially reducing concurrency.
     */

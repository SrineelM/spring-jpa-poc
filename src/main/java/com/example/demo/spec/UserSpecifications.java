...
/**
 * A utility class that creates reusable {@link Specification} objects for querying the {@link User} entity.
 * <p>
 * The {@link Specification} interface is part of Spring Data JPA and is built on top of the
 * JPA Criteria API. It allows you to define query conditions programmatically, resulting in
 * type-safe, dynamic, and reusable query logic.
 * <p>
 * Each method in this class returns a {@code Specification<User>} that encapsulates a single
 * piece of query logic. These specifications can be chained together using {@code .and()} or {@code .or()}
 * to build complex queries in a fluent and readable way.
 * <p>
 * For example, to find all admin users whose name contains "john":
 * <pre>{@code
...
public class UserSpecifications {
     ...
     * @return A {@link Specification} that can be used in a query. Returns a specification that always evaluates to true if the email is null.
     * @see Specification
     */
    public static Specification<User> hasEmail(String email) {
        ...
        // 1. root: A Root<T> object that represents the entity being queried (User in this case). It's used to access entity attributes.
        // 2. query: A CriteriaQuery<?> object that you can use to add ordering or other query details.
        // 3. cb: A CriteriaBuilder object, which is a factory for creating query predicates (the WHERE clause conditions).
        return (root, query, cb) -> {
            ...
            // 1. cb.lower(root.get("email")): Converts the "email" attribute of the User entity to lowercase.
            // 2. email.toLowerCase(): Converts the input email to lowercase.
            // 3. cb.equal(...): Creates an equality condition (e.g., WHERE lower(email) = 'test@example.com').
            return cb.equal(cb.lower(root.get("email")), email.toLowerCase());
        };
    }

    /**
     * Creates a specification to find users whose name contains a given search term (case-insensitive).
     *
     * @param term The search term to look for within the user's name. If null or blank, this specification is ignored.
     ...
     *
     * @param roleStr The role to search for (e.g., "ADMIN", "USER"). If null or blank, this specification is ignored.
     * @return A {@link Specification} for the query.
     * @see Specification
     */

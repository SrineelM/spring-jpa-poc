...
public class AuthController {
    ...
    public ResponseEntity<?> authenticateUser(@RequestBody LoginDto loginDto) {
        ...
        try {
            ...
            logger.warn(
                    "Failed login attempt for email: {} - Validation error: {}", loginDto.getEmail(), e.getMessage());
    ...
    public ResponseEntity<?> registerUser(@RequestBody SignUpDto signUpDto) {
        ...
        try {
            ...
            logger.warn("Registration validation failed for email: {} - {}", signUpDto.getEmail(), e.getMessage());
            return ResponseEntity.badRequest().body(createErrorResponse("VALIDATION_ERROR", e.getMessage()));
    ...
    private void validateRegistrationInput(SignUpDto signUpDto) {
        ...
        if (signUpDto.getName() == null || signUpDto.getName().trim().length() < 2) {
            throw new BusinessRuleViolationException(
                    "Name must be at least 2 characters long", "INVALID_NAME", signUpDto.getName());
        }

        if (signUpDto.getName().trim().length() > 50) {
            throw new BusinessRuleViolationException(
                    "Name cannot exceed 50 characters", "NAME_TOO_LONG", signUpDto.getName());
        }
        ...
        if (!isValidPassword(signUpDto.getPassword())) {
            throw new BusinessRuleViolationException(
                    "Password does not meet security requirements", "WEAK_PASSWORD", null);
        }
    }

...
/**
 * This controller provides endpoints to demonstrate various transaction management strategies. Each
 * endpoint triggers a specific transactional scenario, allowing you to observe the behavior in the
 * application logs and the H2 database console.
 */
@RestController
...
public class TransactionDemoController {
  ...
  /**
   * Demonstrates declarative transaction rollback for a checked exception. A new user is created,
   * but a CustomCheckedException is thrown, which should trigger a rollback. Check the logs and H2
   * console to verify that the user is not saved.
   */
  ...
  public ResponseEntity<String> demoDeclarativeRollback() {
    ...
    try {
      ...
      return ResponseEntity.ok(
          "Demonstrated declarative rollback. Check logs for details. The user should NOT be in the"
              + " database.");
    }
  ...
  /**
   * Demonstrates REQUIRES_NEW propagation. The outer transaction saves user1. The inner
   * transaction, running in a new transaction, saves user2. Even if the inner transaction were to
   * fail, the outer one would still commit. Check the H2 console to verify that both users are
   * saved.
   */
  ...
  public ResponseEntity<String> demoDeclarativePropagation() {
    ...
    return ResponseEntity.ok(
        "Demonstrated REQUIRES_NEW propagation. Both users should be in the database.");
  }

  /**
   * Demonstrates a successful programmatic transaction. Check the H2 console to verify the user is
   * saved.
   */
  ...
  public ResponseEntity<String> demoProgrammaticCommit() {
    ...
    return ResponseEntity.ok(
        "Demonstrated programmatic commit. The user should be in the database.");
  }

  /**
   * Demonstrates a programmatic transaction that is rolled back. Check the H2 console to verify the
   * user is NOT saved.
   */
  ...
  public ResponseEntity<String> demoProgrammaticRollback() {
    ...
    return ResponseEntity.ok(
        "Demonstrated programmatic rollback. The user should NOT be in the database.");
  }

  /**
   * Demonstrates a successful transaction managed by AOP. Check the H2 console to verify the user
   * is saved.
   */
  ...
  /**
   * Demonstrates an AOP-managed transaction that is rolled back. Check the H2 console to verify the
   * user is NOT saved.
   */
  ...
  public ResponseEntity<String> demoAopRollback() {
    ...
    return ResponseEntity.ok(
        "Demonstrated AOP-based rollback. The user should NOT be in the database.");
  }
}
